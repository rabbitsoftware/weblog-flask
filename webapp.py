...
def index():
    return flask.render_template('blog_index.html', posts=content)

@app.route('/feed', methods=['GET'])
def feed():
    lastupdate = max(x['updated'] for x in content)
    resp = flask.Response(flask.render_template('blog_feed.xml', posts=content, lastupdate=lastupdate))
...
def post(name):
    if name in content_mapping:
        return flask.render_template('blog_post.html', post=content_mapping[name])
